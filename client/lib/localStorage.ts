...
export const localStorageService = {
  // Existing methods
  addMoodEntry: async (entry: MoodEntry): Promise<MoodEntry | null> => {
    try {
      const existingData = localStorage.getItem(STORAGE_KEYS.MOOD_ENTRIES);
  ...
  addChatMessage: async (message: ChatMessage): Promise<ChatMessage | null> => {
    try {
      const existingData = localStorage.getItem(STORAGE_KEYS.CHAT_MESSAGES);
  ...
  getAllUserData: () => {
    try {
      return {
        moodEntries: JSON.parse(
          localStorage.getItem(STORAGE_KEYS.MOOD_ENTRIES) || "[]",
        ),
        ...
        userStats: JSON.parse(
          localStorage.getItem(STORAGE_KEYS.USER_STATS) ||
            '{"level":1,"points":0,"currentStreak":0,"longestStreak":0,"totalEntries":0,"totalWords":0,"lastActivity":"' +
        ...
        pointActivities: JSON.parse(
          localStorage.getItem(STORAGE_KEYS.POINT_ACTIVITIES) || "[]",
        ),
      };
    } catch (error) {
  ...
  addPoints: async (
    ...
    try {
      // Update user stats
      const existingStats = localStorage.getItem(STORAGE_KEYS.USER_STATS);
      ...
      userStats.level = Math.max(1, Math.floor(userStats.points / 100));
      ...
      const existingActivities = localStorage.getItem(
        STORAGE_KEYS.POINT_ACTIVITIES,
      );
      const activities = existingActivities
        ? JSON.parse(existingActivities)
        : [];
      activities.unshift(pointActivity);
      localStorage.setItem(
        STORAGE_KEYS.POINT_ACTIVITIES,
        JSON.stringify(activities),
      );
    } catch (error) {
      console.error("Error adding points to localStorage:", error);
    }
  },
};
