    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          result.details.push(
            "✅ RLS security active (requires authentication)",
          );
        } else {
        ...
        try {
          ...
          if (profileError) {
            if (profileError.message.includes("PGRST116")) {
              ...
              result.errors.push(
                `❌ Profile access error: ${profileError.message}`,
              );
            }
          } else if (profileData) {
            result.details.push(
              `✅ Profile data accessible: ${profileData.name}`,
            );
          }
        } catch (e: any) {
          if (e.message.includes("timed out")) {
...
export const formatHealthCheckResults = (result: HealthCheckResult): string => {
  ...
  lines.push("================================");
  lines.push(
    `Configuration: ${result.configured ? "✅ Ready" : "❌ Not Configured"}`,
  );
  lines.push(`Connection: ${result.connected ? "✅ Active" : "❌ Failed"}`);
  lines.push(
    `Authentication: ${result.authenticated ? "✅ Logged In" : "ℹ️ Not Logged In"}`,
  );
  lines.push(
    `Security (RLS): ${result.rlsWorking ? "✅ Active" : "❌ Not Working"}`,
  );
  lines.push("");
