  ...
  try {
    if (isSupabaseConfigured) {
      ...
      if (error) {
        ...
        console.log("🔐 Supabase session:", session ? `User ID: ${session.user.id}` : "No session");
      }
    }
    
    const localUser = localStorageService.getCurrentUser();
    console.log("💾 localStorage user:", localUser ? `Name: ${localUser.name}, Email: ${localUser.email}` : "No local user");
  } catch (err) {
